//Protoype de la fonction Bestiaire
...
void tri(EspecePokemon bestiaire[]) {
	...
	for (int i = 0; i <= 149; i++) {
		...
		for (int j = i + 1; j <= 149; j++) {
			if (strcmp(min.nom, bestiaire[j].nom) > 0) {
				min = bestiaire[j];
				indice = j;
			}
		}
...
void affichageBestiaire(EspecePokemon bestiaire[]) {
	int i = 0;
	short t;
	EspecePokemon fakeBestiaire[150];
	memcpy(fakeBestiaire, bestiaire, sizeof(EspecePokemon) * 150);

	do {
		cout << "\tTapez 1 pour afficher le bestiaire trie ou 0 pour afficher le default" << endl;
		cin >> t;
		if (t != 1 || t != 0) {
			cout << "\tVotre selection n'est pas valide\nVeuillez choisir une option valide" << endl;
			cout << endl;
		}
	} while ( t < 0 || t > 1);
